...
const TBody = (props)=>{
    ...
    return(
        <C.Container>
            ...
            <C.Input onChange={(e)=>{setInput2(modelos.filter(item => item.nome == e.target.value)[0].codigo); load().then(res => console.log(res))}}>{modelos != '' ? modelos.map((item, index)=>(<option key={index}>{item.nome}</option>)) : (<option>Seleciona a marca</option>)}</C.Input>
            ...
            <C.ContainerCard>{todosAnos.length > 0 ? (todosAnos.map((item, index) => (<CardCar key={index} Title={`${item.Modelo} (${item.AnoModelo})`} Value={item.Valor}/>))) : ("Não")}</C.ContainerCard>
        </C.Container>
    )
}
